...
def load_and_plot_results(results_deltagrad, results_adam):
    ...
    v_history = results_deltagrad["variance_history"]  # List of lists (n_runs x iterations)
    r_history = results_deltagrad["r_history"]          # List of lists (n_runs x iterations)
...
# ==============================================================================
# RELIABILITY METRIC (R) ANALYSIS PLOTS
# ==============================================================================
...
# ==============================================================================
# BENCHMARK COMPARISON PLOTS (OPTIMIZER VS OPTIMIZER)
# ==============================================================================


def plot_accuracy_evolution(results_dg, results_adam):
    """Compares Validation Accuracy (or, for VAE tasks, reconstruction loss) evolution across epochs."""
    plt.figure(figsize=(1.71, 1.5))
...
def plot_variance_comparison(results_dg, results_adam):
    ...
    plt.gca().text(0.40, 0.05, stats_text, transform=plt.gca().transAxes,
                   fontsize=10, bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, pad=0.3))
...
def plot_mean_time_per_epoch(adam_runs_stamps, dg_runs_stamps, bin_size=5):
    ...
    plt.gca().text(0.98, 0.45, stats_text, transform=plt.gca().transAxes, fontsize=10,
                   horizontalalignment='right', bbox=dict(facecolor='white', alpha=0.9, pad=0.3))
...
def plot_combined_loss(adam_results, dg_results, adam_label="Adam", dg_label="DeltaGrad"):
    """Plots training loss for both optimizers to visualize convergence speed."""
